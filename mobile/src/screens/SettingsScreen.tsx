    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView style={styles.container} contentContainerStyle={styles.inner} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.section}>
          ...
          <TextInput style={styles.input} placeholder={t('settings.current_password')} placeholderTextColor="#94a3b8"
            secureTextEntry value={oldPw} onChangeText={setOldPw} />
          <TextInput style={styles.input} placeholder={t('settings.new_password')} placeholderTextColor="#94a3b8"
            secureTextEntry value={newPw} onChangeText={setNewPw} />
          <TextInput style={styles.input} placeholder={t('settings.confirm_password')} placeholderTextColor="#94a3b8"
          ...
          <TouchableOpacity style={styles.btn} onPress={handleChangePassword} disabled={saving}>
            {saving ? <ActivityIndicator color="#fff" /> : <Text style={styles.btnText}>{t('settings.update_password')}</Text>}
          </TouchableOpacity>
        </View>
...
const styles = StyleSheet.create({
  ...
  languageButton: { paddingVertical: 8, paddingHorizontal: 11, borderRadius: 9, borderWidth: 1, borderColor: '#e2e8f0', backgroundColor: '#f8fafc' },
  languageButtonActive: { borderColor: TEAL, backgroundColor: '#ccfbf1' },
  languageText: { color: '#64748b', fontSize: 13, fontWeight: '600' },
  languageTextActive: { color: '#0f766e' },
