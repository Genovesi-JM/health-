...
export default function DoctorFinancePage() {
  ...
  const consultCount = (n: number) => `${n} ${n !== 1 ? t('dfin.consult_many') : t('dfin.consult_one')}`;
  const cards = kpis ? [
    { label: t('dfin.today'),   value: kpis.today.amount,   sub: consultCount(kpis.today.count), icon: <DollarSign size={18} />,   color: '#059669' },
    { label: t('dfin.month'),   value: kpis.month.amount,   sub: consultCount(kpis.month.count), icon: <TrendingUp size={18} />,   color: '#3b82f6' },
    { label: t('dfin.pending'), value: kpis.pending.amount, sub: t('dfin.pending_sub'),          icon: <Clock size={18} />,        color: '#d97706' },
    { label: t('dfin.fee'),     value: kpis.platform_fee.amount, sub: kpis.platform_fee.rate,    icon: <ArrowDownRight size={18} />, color: '#6366f1' },
  ] : [];
  ...
  return (
    <div style={{ margin: '0 auto', padding: '1.5rem 1.25rem 4rem' }}>
      <h1 style={{ fontSize: '1.25rem', fontWeight: 800, margin: '0 0 1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
        <DollarSign size={20} style={{ color: '#059669' }} /> {t('dfin.title')}
      </h1>
        ...
        <>
          ...
          {kpis && (
            <div className="card" style={{ padding: '1rem 1.25rem', marginBottom: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(16,185,129,0.06)' }}>
              <span style={{ fontWeight: 700, fontSize: '0.85rem', color: 'var(--text-secondary)' }}>{t('dfin.net')}</span>
              <span style={{ fontWeight: 800, fontSize: '1.15rem', color: '#059669' }}>{kpis.net_total.amount}</span>
            </div>
          )}

          <div className="card" style={{ padding: '1.25rem' }}>
            <div style={{ fontWeight: 700, marginBottom: '1rem', fontSize: '0.88rem' }}>{t('dfin.recent')}</div>
            {txs.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0' }}>
                {txs.map((tx, i) => (
                  <div key={tx.id} style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '0.7rem 0', borderBottom: i < txs.length - 1 ? '1px solid var(--border)' : 'none' }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontWeight: 600, fontSize: '0.85rem' }}>{tx.patient}</div>
                      <div style={{ fontSize: '0.77rem', color: 'var(--text-muted)', textTransform: 'capitalize' }}>{tx.type} · {tx.date}</div>
                    </div>
                    <span style={{ fontWeight: 700, fontSize: '0.9rem' }}>{tx.amount}</span>
                    <span style={{ padding: '0.2rem 0.65rem', borderRadius: 999, fontSize: '0.73rem', fontWeight: 700,
                      background: tx.status === 'Pago' ? 'rgba(16,185,129,0.1)' : 'rgba(234,179,8,0.1)',
                      color: tx.status === 'Pago' ? '#059669' : '#d97706' }}>{tx.status}</span>
                  </div>
                ))}
              </div>
            )}
